...
如果在斜線和小括號之間加上一組大括號和條件式，就會將它變成 Else If。在 If 和 Else 之間可以插入任意個 Else If 敘述。程式會由上到下判斷條件，並在遇到正確的條件時執行該區塊，否則執行 Else 區塊。
...
計數迴圈會將放在中括號內的識別字宣告成一迴圈範圍的變數，讓它從箭頭左邊的表達式的值開始，執行小括號的內容，之後加一，重複以上過程直到它的值超過箭頭右邊的值。
...
使用引入符號 `>>>` 加上 `"模組名稱"` 可以引入模組供程式使用。有什麼模組是由提供編譯器的人決定，HiZollo 官方有自己的一套模組，但任何人都可以製作模組給 HiZollo Script 的編譯器使用。

你不可以引入相同模組兩次，且引入敘述只能放在程式開頭。
...
HiZollo Script 不能讓使用者自訂函式，所有函式都是從外部來的。你可以使用 `函式名稱(參數一, 參數二, ...)` 來呼叫函式。

一個函式如何呼叫取決於該模組的定義，請參見個別模組之定義。
